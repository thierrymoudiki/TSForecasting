...
do_fixed_horizon_local_forecasting <- function(dataset_name, method, input_file_name, 
  ...
  # Loading data from the .tsf file with error handling
  tryCatch({
    loaded_data <- convert_tsf_to_tsibble(
      file.path(BASE_DIR, "tsf_data", input_file_name), 
      VALUE_COL_NAME, 
      key, 
      index
    )
    ...
    # Replace simple counter with progress info
    for(s in seq_along(all_serie_names)) {
      # Print progress percentage every 50 series
      if(s %% 50 == 0) {
        progress <- sprintf("Processing series %d/%d (%.1f%%)", 
                          s, length(all_serie_names), 
                          100 * s/length(all_serie_names))
        print(progress)
      }
